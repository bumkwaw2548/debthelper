...
// ใช้ in-memory database — ไม่มีไฟล์ .db บน disk
// ข้อมูลอยู่ใน RAM ตลอดที่ server ทำงาน ผู้ใช้เห็นและใช้งานได้ปกติ
let _db = null;
...
function getDb() {
  ...
  _db.exec(`
    ...
    INSERT OR IGNORE INTO transactions (user_id, type, category, amount, date, note, is_recurring) VALUES
      (1, 'income',  'salary',    35000, date('now','start of month'), 'เงินเดือน', 1),
      (1, 'expense', 'food',       6000, date('now','start of month'), 'ค่าอาหาร', 1),
      (1, 'expense', 'transport',  2500, date('now','start of month'), 'ค่าเดินทาง', 1),
      (1, 'expense', 'utilities',  1500, date('now','start of month'), 'ค่าสาธารณูปโภค', 1),
      (1, 'expense', 'other',      3000, date('now','start of month'), 'ค่าใช้จ่ายอื่นๆ', 0);
...
function setCorsHeaders(context) {
  ...
  context.res.headers['Access-Control-Allow-Methods'] = 'GET,POST,PUT,DELETE,OPTIONS';
  context.res.headers['Access-Control-Allow-Headers'] = 'Content-Type,Authorization';
}
